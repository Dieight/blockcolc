...
- **环境负载规则（2026-08-16 用户确认）**：全量 E2E、发布门禁、设备构建等性能敏感任务开始前先看机器负载（CPU 负载率、游戏等重负载进程）。后台有明显高负载时**直接停下**，把状态写进工作记忆，等用户腾出机器再继续，不反复重跑撞 flake；环境负载导致的失败不算产品缺陷，不据此改产品代码。
- Playwright 的 `clock.install` 假时钟**会随真实时间流动**（从指定时刻起正常走秒），断言某值保持不动必然赛跑失败；用例应轮询"值在增长/变化"（参照雨丝用例 `data-rain-phase-ms`）。
