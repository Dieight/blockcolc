...
长时间全量测试采用完成事件驱动：启动后等待结束，再读取最终报告和退出码，不做定时进度播报。子代理任务优先等待完成信号；鉴于新版 Codex 偶有子代理停在初始化状态，等待期间每 20 分钟检查一次状态，仅在确实卡住时恢复，不做频繁轮询。需要输入、提前失败或明确异常时及时处理。验证 APK 安装及身份校验结束后，不再操作手机检查 UI，由用户人工验收并反馈。
