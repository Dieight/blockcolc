...
import { AlertTriangle, BarChart3, Check, Clock3, Download, ExternalLink, FileUp, Hammer, History, Info, ListTodo, Map as MapIcon, Pencil, RefreshCw, RotateCcw, Settings, Square, TreePine, Trophy, Upload, X } from 'lucide-react';
...
const WorldCanvasV7 = memo(function WorldCanvasV7({service,resourcePacks,lightingQuality,constructionOutlineVisibility,environmentStyle,worldSeed,terrainGenerationVersion,constructionFeedback=0,focusedProjectId,onSelectProject,onClearWorldFocus}:{service:ApplicationService;resourcePacks:ResourcePackRepository;lightingQuality:VoxelLightingQuality;constructionOutlineVisibility:ConstructionOutlineVisibility;environmentStyle:WorldEnvironmentStyle;worldSeed:string;terrainGenerationVersion:4;constructionFeedback?:number;focusedProjectId:string|null;onSelectProject:(projectId:string)=>void;onClearWorldFocus:()=>void}) {
  ...
  useEffect(()=>{let cancelled=false;let current:VoxelRenderer|null=null;void loadVoxelModule().then(async({createVoxelRenderer,resolveBuiltinBlueprint})=>{if(cancelled||!ref.current)return;current=createVoxelRenderer(ref.current,{resolveBlueprint:id=>importedRef.current.get(id)??resolveBuiltinBlueprint(id),resourcePackAtlasMaximumSize:resourcePackAtlasMaximumSizeForTest(),lightingQuality,constructionOutlineVisibility,environmentStyle,worldSeed,terrainGenerationVersion,onSelectProject:projectId=>selectRef.current(projectId)});renderer.current=current;current.setReducedMotion(matchMedia('(prefers-reduced-motion: reduce)').matches);current.setWorlds(toVoxelWorlds(service.worldProjection().projects,service.snapshot()));current.focusProject(focusRef.current);const pack=await resourcePacks.getActive();if(!cancelled&&current)await current.setResourcePack(pack?{id:pack.id,manifest:pack.manifest}:null);}).catch(error=>console.error('Voxel world initialization failed',error));return()=>{cancelled=true;current?.dispose();if(renderer.current===current)renderer.current=null;};},[service,resourcePacks,lightingQuality,constructionOutlineVisibility,environmentStyle,worldSeed,terrainGenerationVersion]);
...
const WorldCanvas = memo(function WorldCanvas({service,resourcePacks,lightingQuality,constructionOutlineVisibility,constructionFeedback=0}:{service:ApplicationService;resourcePacks:ResourcePackRepository;lightingQuality:VoxelLightingQuality;constructionOutlineVisibility:ConstructionOutlineVisibility;constructionFeedback?:number}) {
  ...
  useEffect(()=>{let cancelled=false;let current:VoxelRenderer|null=null;void loadVoxelModule().then(async({createVoxelRenderer,resolveBuiltinBlueprint})=>{if(cancelled||!ref.current)return;const snapshot=service.snapshot();current=createVoxelRenderer(ref.current,{resolveBlueprint:id=>importedRef.current.get(id)??resolveBuiltinBlueprint(id),resourcePackAtlasMaximumSize:resourcePackAtlasMaximumSizeForTest(),lightingQuality,constructionOutlineVisibility,environmentStyle:snapshot.worldSettings.environmentStyle,worldSeed:snapshot.worldSettings.worldSeed,terrainGenerationVersion:snapshot.worldSettings.terrainGenerationVersion});renderer.current=current;current.setReducedMotion(matchMedia('(prefers-reduced-motion: reduce)').matches);current.setWorlds(toVoxelWorlds(service.worldProjection().projects,snapshot));const pack=await resourcePacks.getActive();if(!cancelled&&current)await current.setResourcePack(pack?{id:pack.id,manifest:pack.manifest}:null);}).catch(error=>console.error('Voxel world initialization failed',error));return()=>{cancelled=true;current?.dispose();if(renderer.current===current)renderer.current=null;};},[service,resourcePacks,lightingQuality,constructionOutlineVisibility,state.worldSettings.environmentStyle,state.worldSettings.worldSeed,state.worldSettings.terrainGenerationVersion]);
  useEffect(()=>{renderer.current?.setWorlds(snapshots);},[snapshots]);
...
function SettingsScreen({service,resourcePacks,state,run,refresh,preferences,onPreferencesChange}:{service:ApplicationService;resourcePacks:ResourcePackRepository;state:ReturnType<ApplicationService['snapshot']>;run:(c:ApplicationCommand)=>Promise<unknown>;refresh:()=>void;preferences:FocusPreferences;onPreferencesChange:(value:FocusPreferences)=>void}) { const date=localDateOf(new Date(),state.calendar.timeZone); const goal=dailyGoalForDate(state,date); const update=(key:'focusMinutes'|'habitFocusMinutes'|'habitTargetRounds'|'breakMinutes',value:number)=>onPreferencesChange({...preferences,[key]:value});return <section className="page"><span className="eyebrow">本地偏好</span><h1>设置</h1><div className="setting time-setting"><div><b>普通任务专注</b><small>下一轮普通任务开始时生效</small></div><label><DeferredNumberInput ariaLabel="普通任务专注分钟" min={1} max={180} value={preferences.focusMinutes} onCommit={value=>update('focusMinutes',value)}/> 分钟</label></div><div className="setting time-setting"><div><b>习惯任务专注</b><small>下一轮习惯任务开始时生效</small></div><label><DeferredNumberInput ariaLabel="习惯任务专注分钟" min={1} max={180} value={preferences.habitFocusMinutes} onCommit={value=>update('habitFocusMinutes',value)}/> 分钟</label></div><div className="setting time-setting"><div><b>每座习惯建筑轮数</b><small>当前建筑完成后，下一座建筑使用新值</small></div><label><DeferredNumberInput ariaLabel="每座习惯建筑轮数" min={10} max={30} value={preferences.habitTargetRounds} onCommit={value=>update('habitTargetRounds',value)}/> 轮</label></div><div className="setting time-setting"><div><b>每轮休息</b><small>两类任务共用；设为 0 可关闭休息</small></div><label><DeferredNumberInput ariaLabel="每轮休息分钟" min={0} max={60} value={preferences.breakMinutes} onCommit={value=>update('breakMinutes',value)}/> 分钟</label></div><FocusIntegritySetting policy={state.focusIntegrityPolicy} run={run}/><NotificationHealthSetting service={service}/><PlannedFocusDaysSetting state={state} run={run}/><div className="setting"><div><b>今日总目标</b><small>{goal.enabled?`${goal.targetPomodoros} 次专注`:'未开启'} · 在任务页调整</small></div></div><div className="setting"><div><b>建筑腐败</b><small>{state.decayPolicy.enabled?'已开启':'默认关闭'}</small></div><button onClick={()=>void run(state.decayPolicy.enabled?{type:'DisableDecay'}:{type:'EnableDecay',damagePerMissedPlannedDayBasisPoints:500,gracePlannedDays:3})}>{state.decayPolicy.enabled?'关闭':'开启'}</button></div><div className="setting environment-style-setting"><div><b>聚落环境</b><small>只改变外围地形，不移动道路和建筑</small></div><div className="segmented-control" role="group" aria-label="聚落环境">{([['natural-valley','自然山谷'],['classic-island','经典空岛']] as const).map(([value,label])=><button key={value} aria-pressed={state.worldSettings.environmentStyle===value} onClick={()=>void run({type:'ConfigureWorldEnvironment',environmentStyle:value})}>{label}</button>)}</div></div><div className="setting lighting-quality-setting"><div><b>光影质量</b><small>自动适配设备；更高档位可能增加耗电与发热</small></div><div className="segmented-control" role="group" aria-label="光影质量">{([['auto','自动','按设备和场景自适应'],['performance','流畅','优先保证交互帧率'],['balanced','均衡','保留阴影和材质层次'],['cinematic','精致','半分辨率光晕与更丰富的材质响应']] as const).map(([value,label,detail])=><button key={value} aria-pressed={preferences.lightingQuality===value} title={detail} onClick={()=>onPreferencesChange({...preferences,lightingQuality:value})}>{label}</button>)}</div></div><div className="setting construction-outline-setting"><div><b>施工轮廓</b><small>控制聚落中未建部分的显示范围</small></div><div className="segmented-control" role="group" aria-label="施工轮廓">{([['off','关闭'],['current','当前'],['all','全部']] as const).map(([value,label])=><button key={value} aria-pressed={preferences.constructionOutlineVisibility===value} onClick={()=>onPreferencesChange({...preferences,constructionOutlineVisibility:value})}>{label}</button>)}</div></div><BuildingBlueprintPanel resources={state.buildingBlueprintResources} run={run}/><ResourcePackPanel repository={resourcePacks}/><BackupPanel service={service} onChanged={refresh}/></section>; }

function NotificationHealthSetting({service}:{service:ApplicationService}) {
  const [capability,setCapability]=useState<NotificationCapability|null>(null);const [failed,setFailed]=useState(false);const [loading,setLoading]=useState(true);
  ...
  return <div className="setting notification-health"><div><b>专注结束提醒</b><small>{status}</small></div><button type="button" aria-label="刷新通知状态" title="刷新通知状态" disabled={loading} onClick={refresh}><RefreshCw className={loading?'is-spinning':''}/></button></div>;
}
