...
export function ResourcePackPanel({repository}:{repository:ResourcePackRepository}) {
  ...
  return <section className="resource-pack-panel" aria-labelledby="resource-pack-title">
    <header><span className="eyebrow">Minecraft Java 16x</span><h2 id="resource-pack-title">方块材质包</h2><p>资源包只改变显示，不会修改蓝图方块、任务或进度。当前任务 JSON 备份不包含资源包文件。</p></header>
    ...
    <p className="resource-pack-license">资源版权归原作者；请只导入你有权使用的资源包。首期支持标准完整立方体、柱体和上下/侧面分材质。</p>
...
function resourcePackErrorMessage(cause:unknown):string{if(cause instanceof ResourcePackError){if(cause.code==='INPUT_TOO_LARGE'||cause.code==='FILE_TOO_LARGE'||cause.code==='TOTAL_UNCOMPRESSED_TOO_LARGE'||cause.code==='TOO_MANY_FILES')return'资源包超过首期安全限制：ZIP 32 MB、单文件 4 MB、总展开 64 MB、最多 4,096 个文件。';if(cause.code==='MISSING_PACK_MCMETA'||cause.code==='INVALID_PACK_MCMETA')return'资源包根目录缺少有效的 pack.mcmeta。';if(cause.code==='UNSAFE_PATH'||cause.code==='DUPLICATE_PATH'||cause.code==='CASE_COLLISION'||cause.code==='ENCRYPTED_ENTRY')return'资源包包含不安全、重复、大小写冲突或加密的文件路径。';return'无法解析这个 Java 资源包 ZIP。';}return cause instanceof Error?cause.message:'资源包操作失败，请重试。';}
