...
export const QUALITY_PROFILES: Readonly<Record<QualityTier, QualityProfile>> = {
  ...
  high: { tier: "high", maxPixelRatio: 1.75, shadowMapSize: 1024, maxLocalLights: 2, maxGlowSprites: 2, weatherDensity: 1, starCount: 960, cloudLobes: 3 },
};

export function selectQualityTier(signals: QualitySignals): QualityTier {
  if ((signals.voxelCount ?? 0) > 60_000 || (signals.maxTextureSize ?? 4096) < 4096
    || (signals.hardwareConcurrency ?? 4) <= 2 || (signals.deviceMemoryGb ?? 4) <= 2) return "low";
  if ((signals.voxelCount ?? 0) > 25_000 || (signals.hardwareConcurrency ?? 8) <= 4
